#!/usr/bin/env python3
"""nanokimi — boucle d'entraînement (train.py)

AdamW + cosine (warmup), CE loss, fenêtres aléatoires sur tokens.bin (uint16).
Checkpoints ATOMIQUES (tmp + rename) toutes les N steps ET toutes les T secondes
(box preemptible : --resume reprend du dernier état sain : model+opt+step+rng).

exemples :
  # smoke local (config réduite) :
  python3 train.py --data out_dev/tokens.bin --out out_dev/ckpt \
      --layers 4 --batch 4 --seq 128 --steps 200 --threads 10
  # vrai run (box 32 vCPU) :
...
def rss_gb():
    """RSS du processus (Go) via /proc/self/status — stdlib pur."""
...
def dev_mem_str(dev):
    ...
    if dev == "mps":
        try:
            return f"mps alloc {torch.mps.current_allocated_memory() / 1e9:.2f} Go"
        except Exception:
            return "mps alloc n/a"
    return f"rss {rss_gb():.1f} Go"
...
def save_ckpt(model, opt, step, rng, args, path):
    ...
    os.replace(tmp, path)  # atomique sur le même filesystem
...
def main():
    ...
    ap.add_argument("--trim-every", type=int, default=0,
                    help="si >0 : gc.collect() + malloc_trim(0) tous les N steps (anti-creep RSS glibc)")
    ap.add_argument("--max-hours", type=float, default=None,
                    help="arrêt propre après N heures (checkpoint final puis sortie)")
    ap.add_argument("--rss-cap", type=float, default=0.0,
                    help="si >0 : arrêt propre (checkpoint) quand RSS dépasse N Go — anti-OOM-kill")
    ...
    loss_ema = None
    toks = 0
    stop_reason = "steps atteints"
    for step in range(step0, args.steps):
        if args.max_hours is not None and (time.time() - t0) > args.max_hours * 3600:
            stop_reason = f"time-cap {args.max_hours} h atteint"
            print(f"[{stop_reason} — arrêt propre au step {step}]", flush=True)
            break
        if args.rss_cap > 0 and rss_gb() > args.rss_cap:
            stop_reason = f"rss-cap {args.rss_cap} Go dépassé ({rss_gb():.1f} Go)"
            print(f"[{stop_reason} — arrêt propre au step {step}]", flush=True)
            break
        lr = lr_at(step, args)
        ...
        # libère explicitement les tenseurs du step (réduit le churn malloc)
        del x, y, logits, loss
        ...
        if (step + 1) % args.log_every == 0 or step == step0:
            ...
            print(
                ...
                f"rss {rss_gb():.1f} Go | {dt / 60:.1f} min, eta {eta / 60:.1f} min",
                flush=True,
            )
        ...
        if (step + 1) % args.ckpt_every == 0 or (now - t_ckpt) > args.ckpt_secs or (step + 1) == args.steps:
            ...
            t_ckpt = now
            print(f"  [ckpt step {step + 1} écrit]", flush=True)

    # checkpoint final systématique (notamment après un time-cap : `step` est
    # l'indice non exécuté = nombre total de steps effectués → --resume correct)
    if stop_reason != "steps atteints":
        save_ckpt(model, opt, step, rng, args, os.path.join(args.out, f"ckpt_{step:07d}.pt"))
        print(f"  [ckpt final step {step} écrit]", flush=True)
    print(f"terminé ({stop_reason}) : {toks / 1e6:.1f} M tokens en {(time.time() - t0) / 60:.1f} min", flush=True)
    if args.bench is not None:
        dt = time.time() - t0
        print(
