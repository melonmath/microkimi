...
The converter fails closed on a different base, an unknown tensor, a shape mismatch, a non-fp32 target, packed routed experts, saved biases, modules-to-save, DoRA, rsLoRA, QALoRA, layer replication, or per-module rank/alpha patterns. This first format is K3-runtime only. `--memory`, `--save`, `prefill`/`absorb`, and the chat prefix cache are disabled with external packs until their state fingerprints include the adapter-set digest. Use `python3 nano/adapter_pack.py inspect skill.mkap` to verify a pack without loading a model.
