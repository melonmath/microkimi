...
- `NANO_ACT_OFFLOAD=1`: the KDA time-segment checkpointing (`NANO_KDA_SEG`) stashes each segment's inputs in a ring of pinned host buffers (async D2H on a side CUDA stream during forward, H2D back before the recompute) instead of retaining them in VRAM. Same math: outputs and grads bit-identical (nano/test_kda_offload_parity.py). Same error guard as NANO_KDA_CHUNKED.
