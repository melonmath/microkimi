...
### External model adapter packs (`nano/adapter_pack.py`, `--adapter`)

`nano/adapter_pack.py create` converts an elementary same-base PEFT LoRA into an `MKADAPT1` `.mkap` file. Each target names one fp32 matrix and carries A, B, rank, scale, factor hashes, and the SHA-256 of the exact base `.bin`. At load, the engine verifies the factors and the complete base file, sorts repeated `--adapter` packs by their own digest, and applies `W <- W + scale * B A` once in float64 accumulation. A mapped model uses private copy-on-write pages, so the base file stays byte-identical; inference then reads ordinary folded matrices and has no adapter-branch cost. Packs may overlap a matrix and compose additively in a deterministic order.

The converter fails closed on a different base, an unknown tensor, a shape mismatch, a non-fp32 target, packed routed experts, saved biases, modules-to-save, DoRA, rsLoRA, QALoRA, layer replication, or per-module rank/alpha patterns. The same format also applies to Qwen f32 spine tensors; see [QWEN.md](QWEN.md). `--memory`, `--save`, `prefill`/`absorb`, and the K3 chat prefix cache are disabled with external packs until their state fingerprints include the adapter-set digest. Use `python3 nano/adapter_pack.py inspect skill.mkap` to verify a pack without loading a model.
