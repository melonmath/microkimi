...
for i in $(seq 1 $1); do
  A=$(MICROKIMI_Q8_SPINE=1 $MK prefillbench --model "$S/q08.bin" --reps 8 2>/dev/null | grep -oE "best [0-9.]+ ms/token \([0-9]+ tok/s\) \| median [0-9.]+ ms/token \([0-9]+" | grep -oE "\([0-9]+" | tr -d '(' | tr '\n' ',' | sed 's/,$//')
  D=$($LB -m "$S/q08.gguf" -p 1024 -n 0 -r 8 -t 10 2>/dev/null | grep pp1024 | grep -oE "[0-9]+\.[0-9]+ ±" | head -1 | cut -d' ' -f1)
  echo "$i,$A,$D"
done
