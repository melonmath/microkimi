...
impl QwenModel {
    ...
    pub fn prefill_collect(&mut self, tokens: &[u32], all_logits: bool) -> QwenPrefillOut {
        ...
        #[cfg(target_os = "macos")]
        let mut done_until = 0usize;
        for l in 0..c.n_layers {
            #[cfg(target_os = "macos")]
            if l < done_until {
                continue; // inside a GPU chain already run
            }
            let layer = &self.layers[l];
            let data = &self.bin.data;
            {
            ...
            // GPU phase 3/5: the run of consecutive dense linear layers
            // starting here as ONE command buffer (input norm, projections,
            // conv, scan, gated norm, out_proj, residual, post-norm, MLP per
            // layer, the f32 residual stream resident for the whole run).
            // The MLP residual of the previous layer was already folded by
            // par_add_norm above, so `hidden` is current.
            #[cfg(target_os = "macos")]
                    ...
                    for i in l..l + n {
                        ...
                        chain.push(crate::model::metal::ChainLayerRef::Linear(crate::model::metal::ChainLayer {
                            w: crate::model::decode_refs::LinLayerRefs {
                                ...
                                a_log: tensor(data, &w.a_log),
                                dt_bias: tensor(data, &w.dt_bias),
                                // the INPUT norm rides in norm_w; the gated
                                // (DeltaNet output) norm weight goes separately
                                norm_w: tensor(data, &li.input_norm),
                                out_proj: tensor(data, &w.out_proj),
                                post_norm_w: tensor(data, &li.post_norm),
                                gate: packed_parts(data, gate),
                                up: packed_parts(data, up),
                                down: packed_parts(data, down),
                            },
